...
extension GameScene {
    ...
    func setupBackground() {
        ...
        background.zPosition = -10  // Задайте досить низьке значення
        background.position = CGPoint(x: frame.midX, y: frame.midY)
        background.size = size  // Задайте розмір фону розміром сцени
        addChild(background)
    }
    ...
    func setupHUD() {
        ...
        // Налаштування поточного балу
        hud.setupScoreLbl(numScore)

        // Налаштування вищого балу
        hud.setupHighscoreLbl(ScoreGenerator.sharedInstance.getHighscore())

        // Розміщення міток балів
        hud.scoreLbl.horizontalAlignmentMode = .left
        hud.scoreLbl.position = CGPoint(x: 20, y: size.height - 300)  // Зменшуємо значення y, щоб опустити мітку нижче

        hud.highscoreLbl.horizontalAlignmentMode = .left
        hud.highscoreLbl.position = CGPoint(x: 20, y: hud.scoreLbl.position.y + 80)  // Збільшуємо проміжок між мітками
    }
